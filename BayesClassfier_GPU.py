...
class BayesianClassifier():
    ...
    def data_generator(self,data_path):
        ...
        # return torch.tensor(features).to(self.device),torch.tensor(labels).to(self.device)
        return np.array(features),np.array(labels)

    def dataset_split(self,train_data,train_target,test_size = 0.3,random_state=None):
        x_train,x_test,y_train, y_test = train_test_split(train_data,train_target,test_size=test_size, random_state=random_state)
        return torch.tensor(x_train).to(self.device),torch.tensor(x_test).to(self.device),torch.tensor(y_train).to(self.device), torch.tensor(y_test).to(self.device)
    ...
    def prior_probility(self,features,labels):
        ...
        for label in tqdm(labels):
            label_str = str(label.item()) #将标签转换为字典的key
    ...
    def Gaussian_distribution(self,x,mu,sigma):
        return torch.tensor(1.0/(2*math.pi)*math.exp(-1.0/2*math.pow((x-mu)*1.0/sigma,2))).to(self.device)
    ...
    def parzen_window_estimate(self,x,x_train,y_train,h1=0.5):
        ...
        for key in labelname:
            index = torch.eq(y_train,float(key))#利用标签获取该类的索引
            class_data = x_train_one[index]#利用该类的索引获取数据
            N = class_data.shape[0]
            CCP = torch.zeros(1).to(self.device)
            for i in range(N):
                # CCP += self.Gaussian_distribution(x,class_data[i],math.sqrt(window_h))*1.0/(N*window_h) #计算Pn(x)
                CCP = torch.add(CCP,self.Gaussian_distribution(x,class_data[i],window_h)*1.0/(N*window_h))
    ...
    def get_parzenwindow_hN(self,h1):
        ...
        via VN = hN = h1/sqrt(N)
        '''
        return torch.tensor(h1/math.sqrt(1.0*len(self.x_train))).to(self.device)
