...
class BayesianClassifier():
    ...
    def class_conditional_probability(self,x):
        ...
        if self.CCP_estimate_method == "parzen-window":
            CCPPF = self.parzen_window_estimate(x,self.x_train,self.y_train)
    ...
    def viz_CCPPF(self):
        ...
        for x in tqdm(x_data):
            CCPPF = self.parzen_window_estimate(x,self.x_train,self.y_train,6)
            for key in self.class_name:
        ...
        plt.title("Parzen窗口估计类条件概率密度 h1=6")
        plt.xlabel("x")#x轴上的名字
        plt.ylabel("p(x|w)")#y轴上的名字
    ...
    def evaluate(self):
        ...
        return acc,prec
BC = BayesianClassifier('./banana.dat',CCP_estimate_method = 'parzen-window')
# x = 1.79
# print(BC.predict(x))
