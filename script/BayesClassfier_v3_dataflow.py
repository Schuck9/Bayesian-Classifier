...
class BayesianClassifier():
    ...
    def parzen_window_estimate(self,x,x_train,y_train,h1=0.5):
        ...
        window_h = self.get_parzenwindow_hN(h1,len(y_train))
        CCPPF = {}
    ...
    def predict(self,x,x_train,y_train,h1):
    ...
    def multi_predict(self,x_tensor,x_train,y_train,h1):
        result = list()
        for x in x_tensor:
            CCPPF = self.class_conditional_probability(x,x_train,y_train,h1)
            posp = self.posterior_probability(x,CCPPF,self.prior_distribution)
    ...
    def evaluate(self,x_test,y_test,x_train,y_train,h1):
        y_pred = []
...
if __name__=="__main__":
    ...
    acc,prec = BC.evaluate(x_test,y_test,x_train,y_train,h1 = 16)#evalutation
